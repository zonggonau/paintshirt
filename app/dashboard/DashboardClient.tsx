    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-8">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white uppercase tracking-tight">Admin Console</h1>
                <button
                    onClick={async () => {
                        await fetch("/api/auth/logout", { method: "POST" });
                        router.push("/login");
                    }}
                    className="text-xs font-bold text-red-500 hover:text-red-700 uppercase tracking-widest px-4 py-2 border border-red-200 rounded-lg transition"
                >
                    Logout
                </button>
            </div>
