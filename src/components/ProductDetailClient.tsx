...
export default function ProductDetailClient({ product }: { product: PrintfulProduct }) {
    ...
    return (
        <div className="flex flex-col space-y-6">
            ...
            {/* Variant Selection */}
            {(uniqueColors.length > 0 || uniqueSizes.length > 0 || variants.length > 1) && (
                <div className="space-y-4 border-t border-b border-gray-200 py-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {uniqueColors.map((color: string) => (
                                    <button
                                        ...
                                        className={`px-4 py-2 border-2 rounded-lg text-sm font-medium transition ${selectedColor === color
                            ...
                            <div className="flex flex-wrap gap-2">
                                {uniqueSizes.map((size: string) => (
                                    <button
                                        ...
                                        className={`px-3 py-2 border-2 rounded-lg text-sm font-bold transition min-w-[3rem] text-center ${selectedSize === size
                                            ? "border-indigo-600 bg-indigo-50 text-indigo-700"
                                            : "border-gray-200 hover:border-gray-300 text-gray-700"
                                            }`}
                                    >
                    ...
                    {/* Fallback Variant Selector (if no color/size parsed but multiple variants exist) */}
                    {uniqueColors.length === 0 && uniqueSizes.length === 0 && variants.length > 1 && (
                        <div>
                            <label className="block text-sm font-semibold text-gray-900 mb-3 uppercase tracking-wider">
                                Option: <span className="text-indigo-600">{activeVariant.name}</span>
                            </label>
                            <div className="space-y-2">
                                {variants.map((v) => (
                                    <button
                                        key={v.external_id}
                                        onClick={() => {
                                            setActiveVariantExternalId(v.external_id);
                                            window.dispatchEvent(new CustomEvent('variantChanged', {
                                                detail: { variantId: v.external_id }
                                            }));
                                        }}
                                        className={`w-full text-left px-4 py-3 border-2 rounded-lg text-sm font-medium transition ${activeVariantExternalId === v.external_id
                                            ? "border-indigo-600 bg-indigo-50 text-indigo-700"
                                            : "border-gray-200 hover:border-gray-300 text-gray-700"
                                            }`}
                                    >
